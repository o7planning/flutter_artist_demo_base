...
import '../filter_criteria/teacher15a_filter_criteria.dart';

class Teacher15aFilterModel
    extends
        FilterModel<
          EmptyFilterInput, //
          Teacher15aFilterCriteria
        > {
  static const String filterName = "teacher15a-filter-model";

  final _universityProvider = UniversityRestProvider();
  ...
  Future<XData?> performLoadMultiOptTildeCriterionXData({
    ...
    if (multiOptTildeCriterionName == "university~") {
      ApiResult<UniversityInfoPage> result = await _universityProvider
          .allUniversities();
      // IMPORTANT: throw an ApiError if Rest API error.
      result.throwIfError();
  ...
  @override
  Teacher15aFilterCriteria createNewFilterCriteria({
    required Map<String, dynamic> tildeCriteriaMap,
  }) {
    return Teacher15aFilterCriteria(
